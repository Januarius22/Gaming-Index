...
import { saveKycSubmission } from "@/actions/seller";
...
export async function POST(request: Request) {
  ...
  const result = await saveKycSubmission({ seller: profile, formData });

  if (result.status === "error") {
    return redirectTo(request, "/seller/kyc", {
        error: result.message,
        open: "1"
      });
  }

  return redirectTo(request, "/seller/dashboard", {
      kyc: "submitted"
    });
}
