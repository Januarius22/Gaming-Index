...
import {
  approveReactivationRequestInlineAction,
  rejectReactivationRequestInlineAction
} from "@/actions/admin";
...
import type { AccountReactivationRequest } from "@/types";

const statusVariant: Record<AccountReactivationRequest["status"], "warning" | "success" | "danger" | "neutral"> = {
  pending: "warning",
  approved: "success",
  rejected: "danger",
  cancelled: "neutral"
...
export default function AdminReactivationRequestsTable({
  requests
}: {
  requests: AccountReactivationRequest[];
}) {
  const router = useRouter();
  const [, startTransition] = useTransition();
  const [visibleRequests, setVisibleRequests] = useState(requests);
  const [approvingRequest, setApprovingRequest] = useState<AccountReactivationRequest | null>(null);
  const [rejectingRequest, setRejectingRequest] = useState<AccountReactivationRequest | null>(null);
  ...
  const submitAction = (formData: FormData, mode: "approve" | "reject") => {
    ...
    startTransition(() => {
      void (async () => {
        const result =
          mode === "approve"
            ? await approveReactivationRequestInlineAction(formData)
            : await rejectReactivationRequestInlineAction(formData);
            ...
            current.map((request) =>
              request.id === result.requestId
                ? {
                    ...
                    admin_note: mode === "reject" ? adminNote : "Account restored after reactivation request.",
                  ...
                  <tr key={request.id} className="border-b border-border/60 align-top">
                    ...
                    <td className="px-4 py-4">
                      <p className="max-w-md text-sm leading-6 text-foreground">
                        {request.reason || "Reactivation requested."}
                      </p>
                      {request.admin_note ? (
                    ...
                    <td className="px-4 py-4">
                      {isPending ? (
                        ...
                        <span className="text-xs font-semibold text-muted-foreground">Reviewed</span>
                      )}
                    </td>
                  </tr>
                );
        ...
        {approvingRequest ? (
          ...
          >
            ...
            <div className="rounded-3xl border border-border bg-surface p-4 text-sm leading-6 text-muted-foreground">
              This restores account access and sends the user a notification.
            </div>
            <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              <Button type="button" variant="secondary" onClick={() => setApprovingRequest(null)}>
                Cancel
              </Button>
              <Button type="submit" disabled={pendingId === approvingRequest.id} className="gap-2">
                <RotateCcw className="h-4 w-4" />
                {pendingId === approvingRequest.id ? "Restoring..." : "Restore account"}
              </Button>
            </div>
          </form>
        ) : null}
          ...
          >
            ...
            <label className="block space-y-2">
              ...
              <textarea
                ...
                required
                placeholder="Explain why this account cannot be restored now."
